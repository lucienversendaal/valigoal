...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" class="light">
    ...
    <body class="min-h-screen bg-white dark:bg-zinc-800">
        <flux:sidebar sticky collapsible="mobile" class="border-e border-zinc-200 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900">
            ...
            <flux:sidebar.nav>
                <flux:sidebar.group :heading="__('Poule')" class="grid">
                    ...
                    <flux:sidebar.item icon="share" :href="route('bracket')" :current="request()->routeIs('bracket')" wire:navigate>
                        {{ __('Knockout') }}
                    </flux:sidebar.item>
                    <flux:sidebar.item icon="star" :href="route('bonus')" :current="request()->routeIs('bonus')" wire:navigate>
                        {{ __('Bonusvragen') }}
                    </flux:sidebar.item>
            ...
            @if (auth()->user()?->isSuperAdmin())
                <flux:sidebar.nav>
                    <flux:sidebar.item icon="shield-check" href="/admin" target="_blank">
                        {{ __('Beheer') }}
                    </flux:sidebar.item>
                </flux:sidebar.nav>
            @endif
